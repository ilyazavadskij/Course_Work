...
public class WavHeaderReader {
     ...
     *
     * @return com.stc.mp.integration.WavHeader object
     * @throws IOException
     * @see WavHeader
     */
    ...
    private int toInt(int start, boolean endian) {
        ByteBuffer bb = ByteBuffer.wrap(buf, start, 4);
        bb.order(ByteOrder.LITTLE_ENDIAN);//TODO
        return bb.getInt();
        /*
        int k = (endian) ? 1 : -1;
        if (!endian) {
    ...
    /**
     * Return com.stc.mp.integration.WavHeader object which contain wave header
     *
     * @return com.stc.mp.integration.WavHeader object
     */
